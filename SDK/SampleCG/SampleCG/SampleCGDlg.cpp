...
#if defined(_M_X64)
	::_stprintf_s( tc_text, _T("SampleCG-64Bits, (Version : %s)"), pApp->m_szFileVersion.GetString() );
#elif defined(_M_IX86)
	::_stprintf_s( tc_text, _T("SampleCG-32Bits, (Version : %s)"), pApp->m_szFileVersion.GetString() );
#else
	DebugBreak();
#endif
	...
	case	IDC_BTN_ROISETUP	:
		{
			if ( BN_CLICKED == _notify_code ) {
				...
				num = (int)::SendMessage( ::GetDlgItem( this->GetSafeHwnd(), IDC_CMB_ROINUM ), CB_GETCURSEL, 0L, 0L );
	...
	case	IDC_CMB_PALETTE		:
		{
			if ( CBN_SELCHANGE == _notify_code ) {
				...
				if ( CB_ERR == res ) {
					...
					m_stTCamInfo.set_sel_palette( (int)res );
				}
	...
	case	IDC_CMB_TEMPMODE	:
		{
			if ( CBN_SELCHANGE == _notify_code ) {
				...
				res = ::SendMessage( _h_wnd, CB_GETCURSEL, 0L, 0L );
				res = ::SendMessage( _h_wnd, CB_GETITEMDATA, res, 0L );
				if ( CB_ERR == res ) {
				}
				else {
					int				msg_res;
					...
					m_stTCamInfo.set_temp_mode( (int)res );
...
#ifdef	_UNICODE
	::MultiByteToWideChar( CP_ACP, 0, _model, (int)::strlen(_model), model, 32 );
	::MultiByteToWideChar( CP_ACP, 0, _ip, (int)::strlen(_ip), ip, 32 );
	::MultiByteToWideChar( CP_ACP, 0, _mac, (int)::strlen(_mac), mac, 32 );
#else
	::strcpy( model, _model );
	::strcpy( ip, _ip );
	::strcpy( mac, _mac );
...
void	CSampleCGDlg::DoRecvCamData( TCAMINFO_PTR _p_info )
{
	...
	else if( XGA_ID == p_cfg_data->sensor ) {
		_p_info->ir_size.xSize = 1024;
		_p_info->ir_size.ySize = 768;
	}
	else {
	...
	if ( 0 == p_cfg_data->support_temp_mode ) {
		HWND			h_ctrl_wnd = ::GetDlgItem( this->GetSafeHwnd(), IDC_CMB_TEMPMODE );
...
#if 1
		{
			...
			TCHAR			temp_name[2][10] = { { 0, }, { 0, } };
			PCHAR			p_temp;


			::SendMessage( h_ctrl_wnd, CB_RESETCONTENT, 0L, 0L );

			p_temp = (PCHAR)&p_cfg_data->meas_name[0][0];
			if ( 0 < ::strlen(p_temp) ) {
				::MultiByteToWideChar( CP_ACP, 0, p_temp, (int)::strlen(p_temp), &temp_name[0][0], 10 );
				::SendMessage( h_ctrl_wnd, CB_ADDSTRING, 0L, (LPARAM)(LPCTSTR)temp_name[0] );

				//	check medical mode
				if ( p_cfg_data->support_temp_mode & (0x0001 << 2) ) {
					::SendMessage( h_ctrl_wnd, CB_SETITEMDATA, 0L, (LPARAM)2 );
				}
				else {
					::SendMessage( h_ctrl_wnd, CB_SETITEMDATA, 0L, (LPARAM)0 );
				}
			}

			p_temp = (PCHAR)&p_cfg_data->meas_name[1][0];
			if ( 0 < ::strlen(p_temp) ) {
				::MultiByteToWideChar( CP_ACP, 0, p_temp, (int)::strlen(p_temp), &temp_name[1][0], 10 );
				::SendMessage( h_ctrl_wnd, CB_ADDSTRING, 0L, (LPARAM)(LPCTSTR)temp_name[1] );
				::SendMessage( h_ctrl_wnd, CB_SETITEMDATA, 1L, (LPARAM)1 );
			}

			::SendMessage( h_ctrl_wnd, CB_SETCURSEL, 0, 0L );
		}
#endif		
		if ( 0 == p_cfg_data->temp_mode || 2 == p_cfg_data->temp_mode ) {
...
void	CSampleCGDlg::DoRecvStreamData( TCAMINFO_PTR _p_info )
{
	...
	{	//	Camera ROI Information
		...
		roi_num		= (int)::SendMessage( ::GetDlgItem(GetSafeHwnd(), IDC_CMB_ROINUM), CB_GETCURSEL, 0L, 0L );
		roi_mode	= (int)::SendMessage( ::GetDlgItem(GetSafeHwnd(), IDC_CMB_ROIMODE), CB_GETCURSEL, 0L, 0L );
