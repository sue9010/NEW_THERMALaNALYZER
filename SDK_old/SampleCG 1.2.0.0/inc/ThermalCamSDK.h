...
typedef struct
{	
	unsigned short			xSize;					//	horizontal size of raw data
	unsigned short			ySize;					//	vertical size of raw data
}	IRF_IMAGE_INFO_T;
...
typedef struct strSAVEDATA
{
	union {
		struct
		{
			...
			unsigned char		tv;					//	Video Output Format ( 0 : NTSC, 1 : PAL )
			unsigned char		temp_mode;			//	Temperature Measurement Mode ( 0 : Normal, 1 : High, 2 : Medical )

			unsigned char		id;					//	RS485 ID
			unsigned char		baudrate;			//	RS485 Baudrates
			short				level;				//	CX Model Only
			unsigned short		span;				//	CX Model Only
			unsigned char		agc;				//	AGC MODE ( 1 : AUTO, 0 : MANUAL )
			unsigned char		invert;				//	Video Color Invert ( 0 : OFF, 1 : LUMA, 2 : CHROMA, 3 : L + C )
			unsigned char		mirror;				//	Video Mirror ( 0 : OFF, 1 : ON )
			unsigned char		flip;				//	Video Flip ( 0 : OFF, 1 : ON )
			unsigned char		colorbar;			//	Color-Bar ( 0 : OFF, 1 : ON )
			unsigned char		showinfo;			//	Show Temperature Information ( 0 : OFF, 1 : ON )
			unsigned char		indicator;			//	High/Low Indicator ( 0 : OFF, 1 : ON )
			unsigned char		unit;				//	Temperature Unit ( 0 : Celcius, 1 : Fahrenheit )
			unsigned char		dhcp;				//	DHCP Enable ( 0 : OFF, 1 : ON )
			unsigned char		color;				//	Color Pallete ( reference IRF_CAM_PALETTE_TYPE_T )
													//	( 0 : GREY, 1 : RAINBOW, 2 : IRON, 3 : GREYRED, 4 : GLOWBOW, 5 : YELLOW, 6 : MIDGREY, 7 : MIDGREEN )
			unsigned char		alpha;				//	Alpha Blending ( 0 ; OFF, 1 : 20%, 2 : 40%, 3 : 60%, 4 : 80% )
			unsigned char		zoom;				//	Video Zoom ( 0 : OFF, 1 : x2, 2 : x4 )
			unsigned char		sharp;				//	CX Model Only
			unsigned char		noise;				//	Noise Filter ( 0 : OFF, 1 : NR1, 2 : NR2, 3 : NR1 + NR2 )
			unsigned short		nuc;				//	CX Model Only
			unsigned char		econt;				//	CX Model Only
			unsigned int		ipaddr;				//	IP Address ( ex : 192.168.0.100 => 0xC0A80064 )
			unsigned int		netmask;			//	Subnet Mask ( ex : 255.255.255.0 => 0xFFFFFF00 )
			unsigned int		gateway;			//	Gateway ( ex : 192.168.0.1 => 0xC0A80001 )
			unsigned int		dns;				//	Not Used

			unsigned char		alarm1_func;		//	CX Model Only
			unsigned char 		alarm1_cond;		//	CX Model Only
			short				alarm1_value;		//	CX Model Only
			unsigned char		alarm2_func;		//	CX Model Only
			unsigned char		alarm2_cond;		//	CX Model Only
			short				alarm2_value;		//	CX Model Only
			unsigned char		down_filter;		//	CX Model Only
			unsigned char		show_center;		//	Show Center Cross ( 0 : OFF, 1 : ON )
			unsigned char		show_spot;			//	Show Spot For CX Model ( 0 : OFF, 1 : ON ) 
													//	Show ROI For CG Model ( 0 : OFF, 1 : ON ) 
			unsigned char		show_correction;	//	Show Correction Parameters ( 0 : OFF, 1 : ON ) 
	...
	union {
		struct
		{
			unsigned char		limit9;				//	9Hz Mode
			unsigned char		enable_high;		//	CX Model Only
			unsigned char		correction;			//	Correction Parameters ( 0 : Not Used, 1 : Used )
			unsigned char		emissivity;			//	Emissivity ( 1 ~ 100 )
			unsigned char		transmission;		//	Transmission ( 1 ~ 100 )
			short				atmosphere;			//	Atmosphere (-500 ~ 1000 )
...
typedef struct
{
	...
	unsigned short			fhSize;					// Size of header 
	...
	unsigned int			totalFrame;				// Total frame number 
...
typedef struct 
{
	...
	unsigned int			core_type;				//	core type ( 0 : QVGA, 1 : VGA )
	unsigned short			h_res;					//	horizontal resolution
	unsigned short			v_res;					//	vertical resolution
	//++
...
typedef struct {
	...
	unsigned int			fw_ver;					// Firmware version (Package)
	unsigned int			cpu_ver;				// Firmware version (CPU)
	unsigned int			fpga_ver;				// Firmware version (FPGA)
	unsigned int			engine_ver;				// Firmware version (Engine)
...
typedef struct {
	short					normal_min;				// Minimum temperature in the normal temperature table
	short					normal_max;				// Maximum temperature in the normal temperature table
	short					high_min;				// Minimum temperature in the high temperature table
	short					high_max;				// Maximum temperature in the high temperature table

	unsigned int			normal_table[256];		// Normal temperature table ( normal_max - normal_min + 1 ) * 4bytes(level)
	unsigned int			high_table[256];		// High temperature table ( ( high_max - high_min ) / 5 + 1 ) * 4bytes(level)
}	IRF_TEMP_TABLE_T;
...
//	Common Interface
THERMALSDK_API short PASCAL OpenConnect(HANDLE *pHandle, UINT *pTimerID, LPCSTR strDestination, LPCSTR strServiceName, int nProtocol, int nType);
THERMALSDK_API short PASCAL CloseConnect(HANDLE *handle, UINT timerID);

THERMALSDK_API short PASCAL GetIRHeader(HANDLE handle, IRF_IR_FILE_HEADER_T* header, unsigned long *curPos);
...
THERMALSDK_API short PASCAL SendCameraMessage(HANDLE handle, UINT *pTimerID, IRF_MESSAGE_TYPE_T type, unsigned short PMSGTYPE, unsigned short RCODE);
THERMALSDK_API short PASCAL SendMessageToCamera(HANDLE handle, UINT *pTimerID, IRF_MESSAGE_TYPE_T type, unsigned short PMSGTYPE, unsigned short RCODE, DWORD RCODE2, DWORD RCODE3, DWORD RCODE4);

THERMALSDK_API short PASCAL GetIRImages(HANDLE handle, UINT *pTimerID, IRF_IR_CAM_DATA_T* cam_data);

THERMALSDK_API short PASCAL GetImageLUT(unsigned char *palette, IRF_PALETTE_TYPE_T paletteType, bool bInvert);
THERMALSDK_API short PASCAL GetGrayToPaletteImage(unsigned char *from_image, void* to_image, unsigned short width, unsigned short height, unsigned char *palette, int BitsPixel, BOOL bMirror, BOOL bFlip);
THERMALSDK_API short PASCAL GetIRHistogram(unsigned int *hist, unsigned short *ir_image, long image_size);


//	CX320 Only Interface
THERMALSDK_API float PASCAL GetCorrectedTemp(float *tempLUT, IRF_TEMP_CORRECTION_PAR_T corrPara, unsigned short engineOut);
THERMALSDK_API short PASCAL GetCamTempTable(HANDLE handle, UINT *pTimerID, IRF_RECEIVED_TEMP_TABLE_T* temp_table);
THERMALSDK_API short PASCAL GetTempRangeValue(IRF_DYNAMIC_RANGE_T tempMode, short *min, short *max);

//	CX320, CX640 Only Interface
THERMALSDK_API short PASCAL GetTempMapTable(float* tempLUT, IRF_DYNAMIC_RANGE_T tempMode);
THERMALSDK_API short PASCAL GetImage(unsigned char *image, HANDLE ir_image, long image_size, float *tempLUT, float *level, float *span, IRF_AUTO_RANGE_METHOD_T *method);
THERMALSDK_API short PASCAL GetCorrectedImage(unsigned char *image, HANDLE ir_image, long image_size, float* tempLUT, IRF_TEMP_CORRECTION_PAR_T corrPara, float *level, float *span, IRF_AUTO_RANGE_METHOD_T *method);
...
THERMALSDK_API short PASCAL GetROITemp(HANDLE ir_image, IRF_IMAGE_INFO_T image_info, float *tempLUT, IRF_TEMP_CORRECTION_PAR_T corrPara, RECT roi, IRF_NUMERIC_INFO_T *numInfo, POINT *min_pt, POINT *max_pt);
THERMALSDK_API short PASCAL GetRawToTemp(HANDLE ir_image, IRF_IMAGE_INFO_T image_info, float *tempLUT, IRF_TEMP_CORRECTION_PAR_T corrPara, float* tempImage);
THERMALSDK_API short PASCAL SendPaletteMessage(HANDLE handle, UINT *pTimerID, IRF_SET_USER_PALETTE struct_palette);
THERMALSDK_API short PASCAL GetCameraTempRangeValue( IRF_DYNAMIC_RANGE_T tempMode, IRF_CAM_MODEL_TYPE_T camType, short *min, short *max );

//	CG320, CG640 Only Interface
THERMALSDK_API short PASCAL GetImageCG( unsigned char *image, HANDLE ir_image, long image_size, float *level, float *span, IRF_AUTO_RANGE_METHOD_T *method );
...
THERMALSDK_API short PASCAL GetROITempCG( HANDLE ir_image, IRF_IMAGE_INFO_T image_info, IRF_TEMP_CORRECTION_PAR_T corrPara, RECT roi, IRF_NUMERIC_INFO_T *numInfo, POINT *min_pt, POINT *max_pt );
THERMALSDK_API short PASCAL GetRawToTempCG( HANDLE ir_image, IRF_IMAGE_INFO_T image_info, IRF_TEMP_CORRECTION_PAR_T corrPara, float* tempImage );
THERMALSDK_API float PASCAL ConvertRawToTempCG( HANDLE _ir_image, IRF_TEMP_CORRECTION_PAR_T _corr, unsigned short _raw );
THERMALSDK_API unsigned short PASCAL ConvertTempToRawCG( HANDLE _ir_image, IRF_TEMP_CORRECTION_PAR_T _corr, float _temp );
THERMALSDK_API short PASCAL GetTempRangeValueCG( HANDLE _ir_image, IRF_DYNAMIC_RANGE_T tempMode, short *min, short *max );

//	Image Filter Interface
THERMALSDK_API short PASCAL ApplyImageFilter(unsigned char *image, unsigned short width, unsigned short height, IRF_IMAGE_FILTER_T filter);
THERMALSDK_API short PASCAL ApplyColorImageFilter(void* image, unsigned short width, unsigned short height, IRF_IMAGE_FILTER_T filter, int bitPixel);
THERMALSDK_API void PASCAL BilateralFilter(unsigned char *image, unsigned short width, unsigned short height, float sigD, float sigR, int w);
THERMALSDK_API void PASCAL GetGaussianKernel(int *kernel, int *mult, int sz);
THERMALSDK_API short PASCAL FastGaussianBlur(BYTE *img, int iw, int ih, int *Gkernel, int *Gmult, int radius);
THERMALSDK_API short PASCAL FastStackBlur(BYTE* img, int w, int h, int radius);
THERMALSDK_API short PASCAL BoxBlur(BYTE *src, int src_w, int src_h, int sz);

//	CRD File Interface
THERMALSDK_API short PASCAL CreateIRCamDataCG( IRF_IR_CAM_DATA_T* _p_cam_data );
THERMALSDK_API short PASCAL CloseIRCamDataCG( IRF_IR_CAM_DATA_T* _p_cam_data );
THERMALSDK_API short PASCAL LoadIRCamDataCG( IRF_IR_CAM_DATA_T* _p_cam_data, unsigned short* _p_ir_buf, long _image_size );
THERMALSDK_API short PASCAL LoadIRImage(HANDLE *handle, char *FileName, unsigned long *totSize);
THERMALSDK_API short PASCAL GetIRImageFromStream(HANDLE handle, unsigned short* ir_image, long image_size, unsigned long totStreamSize, unsigned long *curPos, int* gap_time, bool bLoop);
THERMALSDK_API short PASCAL GetIRImageFromStream_n(HANDLE handle, unsigned short* ir_image, long image_size, unsigned long totStreamSize, unsigned long *curPos, int* gap_time, bool bLoop, bool new_ver);	// version < 20
THERMALSDK_API short PASCAL GetIRImageFromStream_v2(HANDLE handle, unsigned short* ir_image, long image_size, unsigned long totStreamSize, unsigned long *curPos, int* gap_time, __int64* curTime, bool bLoop, unsigned char ver);	// version >= 20
THERMALSDK_API short PASCAL GetRevIRImageFromStream(HANDLE handle, unsigned short* ir_image, long image_size, unsigned long *curPos, int* gap_time);
THERMALSDK_API short PASCAL GetRevIRImageFromStream_n(HANDLE handle, unsigned short* ir_image, long image_size, unsigned long *curPos, int* gap_time, bool new_ver);
THERMALSDK_API short PASCAL GetRevIRImageFromStream_v2(HANDLE handle, unsigned short* ir_image, long image_size, unsigned long *curPos, int* gap_time, __int64* curTime, unsigned char ver);
THERMALSDK_API short PASCAL SaveIRImage(HANDLE *handle, char* filename, IRF_IR_FILE_HEADER_T *pHeader);
THERMALSDK_API short PASCAL SaveIRHeader(HANDLE *handle, char* filename, IRF_IR_FILE_HEADER_T *pHeader, IRF_IR_DATA_HEADER_T *pAddedData);
THERMALSDK_API short PASCAL SetIRImageToStream(HANDLE handle, unsigned short* ir_image, long image_size, int millisecond, short *frameCnt);	// file version < 20
THERMALSDK_API short PASCAL SetIRImageToStream_v2(HANDLE handle, unsigned short* ir_image, long image_size, int millisecond, short *frameCnt, unsigned char ver);
THERMALSDK_API short PASCAL CloseIRStream(HANDLE handle);
THERMALSDK_API void PASCAL DrawColorBar(HWND hWnd, HDC hDC, unsigned char* palette, float level, float span, IRF_TEMP_MODE_T tempUnit, bool bUpdateOnlyTickArea);
