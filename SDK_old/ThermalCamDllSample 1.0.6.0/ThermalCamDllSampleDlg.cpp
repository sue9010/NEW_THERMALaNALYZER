	...
	, m_f_LiveImage(TRUE)
{
	...
	cam_data.ir_image = new unsigned short[imageInfo.xSize*imageInfo.ySize*IRF_MAX_IR_BUFFER_SIZE];
	cam_data.image_buffer_size = imageInfo.xSize*imageInfo.ySize;
	cam_data.lpNextData = new BYTE[IRF_BUFFER_SIZE];
	cam_data.dwSize = 0L;
	cam_data.dwPosition = 0L;
...
BOOL CThermalCamDllSampleDlg::OnInitDialog()
{
	...
	::_stprintf_s( tc_text, _T("ThermalCamDllSample, (Version : %s)"), pApp->m_szFileVersion.GetString() );
...
void CThermalCamDllSampleDlg::OnDestroy()
{
	...
	if(cam_data.ir_image != NULL)	delete [] cam_data.ir_image;
	if(cam_data.lpNextData != NULL)	delete [] cam_data.lpNextData;
...
{
	if(cam_data.save_data.alarm1_func > 4)	m_Alarm_Func1 = 0;
	else									m_Alarm_Func1 = cam_data.save_data.alarm1_func;

	if(cam_data.save_data.alarm1_cond > 1)	m_AlarmCond1 = 0;
	else									m_AlarmCond1 = cam_data.save_data.alarm1_cond;

	m_AlarmVal1 = (int)(cam_data.save_data.alarm1_value*0.1);
...
void CThermalCamDllSampleDlg::ImageDisplay(CDC * dc, int x, int y, int width, int height, double StretchFactorX, double StrectchFactorY)
{
	...
	if (BitsPixel==32) {	// 32 bits	
		...
		GetGrayToPaletteImage(m_image, h, width, height, palette, BitsPixel, m_bImgMirror, m_bImgFlip);	// ThermalCamDll
		...
		GetGrayToPaletteImage(m_image, h, width, height, palette, BitsPixel, m_bImgMirror, m_bImgFlip);	
...
void CThermalCamDllSampleDlg::MakePalette(IRF_PALETTE_TYPE_T type, BOOL paletteInv)
{
	...
	if(GetImageLUT(palette, type, m_bPaletteInv) != IRF_NO_ERROR)
	{
		MessageBox(_T("Palette File Open Error!!"), _T("Error Message"), MB_OK);
	}	
}
...
void CThermalCamDllSampleDlg::SetTemperatureTable(int temp_mode)
{
	...
	int index = 0;
	int engine_out, old_out;
	float temperature, old_temp;
	int temp_index = -20;
...
void CThermalCamDllSampleDlg::Run()
{
	...
	memset(&cpy_cam_data, 0x00, sizeof(IRF_IR_CAM_DATA_T));
	memset(cam_data.lpNextData, 0, IRF_BUFFER_SIZE);
	cam_data.dwSize = 0L;
	cam_data.dwPosition = 0L;
	...
	{
		err = GetIRImages(m_hComm, &keepAliveID, &cam_data);
		if(err != IRF_NO_ERROR) {
			SendMessage(MY_MESSAGE1, 0, err);	// Notified error.
			break;
		}

		switch ( cam_data.msg_type ) {
			case	_IRF_CAM_DATA		:	DoRecvCamData( cam_data );		break;
			case	_IRF_SYS_INFO		:	DoRecvSysInfo( cam_data );		break;
			case	_IRF_STREAM_DATA	:	DoRecvStreamData( cam_data );	break;
		}
	}
}
...
void CThermalCamDllSampleDlg::SetImageInfo(unsigned short image_x, unsigned short image_y)
{
	...
	if (cpy_cam_data.image_buffer_size != image_x * image_y)
	{
		delete[] cam_data.ir_image;
		delete[] m_image;


		if ( SAVEDATA_VER_20 < cpy_cam_data.save_data.ver ) {
			cam_data.ir_image = new unsigned short[image_x * ( image_y + 1 ) * IRF_MAX_IR_BUFFER_SIZE];
			cam_data.image_buffer_size = image_x * ( image_y + 1 );
		}
		else {
			cam_data.ir_image = new unsigned short[image_x * image_y * IRF_MAX_IR_BUFFER_SIZE];
			cam_data.image_buffer_size = image_x * image_y;
		}
...
void CThermalCamDllSampleDlg::OnBnClickedAlarmSetBtn()
{
	...
	if ( CX320 == ir_model_type || CX640 == ir_model_type ) {
		// CX
		SendMessageToCamera( m_hComm, &keepAliveID, _IRF_SET_CAM_DATA, CMD_ALARM1_FUNC, m_Alarm_Func1, 0, 0, 0 );
		SendMessageToCamera( m_hComm, &keepAliveID, _IRF_SET_CAM_DATA, CMD_ALARM1_COND, m_AlarmCond1, 0, 0, 0 );
		SendMessageToCamera( m_hComm, &keepAliveID, _IRF_SET_CAM_DATA, CMD_ALARM1_VAL, m_AlarmVal1*10, 0, 0, 0 );
...
void CThermalCamDllSampleDlg::OnBnClickedFileOpenButton()
{
	...
	if(fileDlg.DoModal() == IDOK)
	{
		...
		if(LoadIRImage(&file_handle, buffer, &totSizeIRStream) != IRF_NO_ERROR)
			MessageBox(_T("File load error!"), _T("Error Message"), MB_OK);

		GetIRHeaders(file_handle, pFileHeader, pFileAddHeader, &curPosition);
